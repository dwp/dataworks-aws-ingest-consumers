...
resource "aws_cloudwatch_metric_alarm" "processed_k2hb_batches_under_threshold_equalities" {
  ...
  alarm_name          = "K2HB equalities - Processed batches under 20 in 11 of last 12 hours"
  alarm_description   = "Managed by ${local.common_tags.Application} repository"
  alarm_actions       = [local.monitoring_topic_arn]
...
resource "aws_cloudwatch_metric_alarm" "processed_k2hb_batches_under_threshold_equalities_saturday" {
  ...
  namespace           = local.cw_k2hb_main_agent_namespace
  alarm_name          = "K2HB equalities - Processed batches under 20 in 20 of last 21 hours (Saturday)"
  alarm_description   = "Managed by ${local.common_tags.Application} repository"
  alarm_actions       = [local.monitoring_topic_arn]
...
resource "aws_cloudwatch_metric_alarm" "k2hb_running_tasks_less_than_desired_equalities" {
  count               = local.k2hb_alarm_on_running_tasks_less_than_desired_equalities[local.environment] ? 1 : 0
  alarm_name          = "K2HB equalities - Running tasks less than desired tasks for 15 minutes"
  alarm_description   = "Managed by ${local.common_tags.Application} repository"
  alarm_actions       = [local.monitoring_topic_arn]
  treat_missing_data  = "breaching"
  ...
  metric_query {
    ...
    metric {
      ...
      dimensions = {
        AutoScalingGroupName = aws_autoscaling_group.k2hb_equality_london.name
      }
    }
  }
  ...
  tags = merge(
    local.common_tags,
    {
      Name              = "k2hb-running-tasks-less-than-desired-equalities",
      notification_type = "Warning",
      severity          = "High"
    },
  )
