...
resource "aws_launch_template" "k2hb_equality" {
  ...
  user_data = base64encode(templatefile("k2hb_userdata.tpl", {
    ...
    k2hb_rds_table_name                              = local.metadata_store_table_names.equality
    ...
    k2hb_manifest_prefix                             = local.k2hb_manifest_folder_equality
...
resource "aws_cloudwatch_log_group" "k2hb_ec2_equality_logs" {
  name              = "/aws/ec2/main/k2hb_equality"
  retention_in_days = 180
  tags              = local.common_tags
}

# Old ECS log group - left as CI can't delete it. DW-4728 to remove
resource "aws_cloudwatch_log_group" "k2hb_equality_logs" {
  name              = "/aws/ecs/main/k2hb_equality"
  retention_in_days = 180
  tags              = local.common_tags
}
