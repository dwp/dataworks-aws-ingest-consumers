...
locals {
  k2hb_audit_ireland_tags_asg = merge(
    local.k2hb_audit_tags_asg,
    {
      Name     = "k2hb-audit-ireland"
      Location = "Ireland",
    }
  )
}

resource "aws_launch_template" "k2hb_audit_ireland" {
  name                   = "k2hb_audit_ireland"
  ...
  tags = local.k2hb_audit_ireland_tags_asg
...
resource "aws_autoscaling_group" "k2hb_audit_ireland" {
  name_prefix               = "${aws_launch_template.k2hb_audit_ireland.name}-lt_ver${aws_launch_template.k2hb_audit_ireland.latest_version}_"
  min_size                  = local.k2hb_asg_min[local.environment]
  desired_capacity          = var.k2hb_audit_ireland_asg_desired[local.environment]
  max_size                  = var.k2hb_audit_ireland_asg_max[local.environment]
  ...
  launch_template {
    id      = aws_launch_template.k2hb_audit_ireland.id
    version = "$Latest"
  }

  tags = [
    for key, value in local.k2hb_audit_ireland_tags_asg :
    {
      key                 = key
      value               = value
