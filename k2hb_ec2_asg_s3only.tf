...
locals {
  k2hb_s3only_london_tags_asg = merge(
    local.k2hb_main_tags_asg,
    {
      Name     = "k2hb-s3only-london",
      Location = "London",
    }
  )
}

resource "aws_launch_template" "k2hb_s3only_london" {
  name                   = "k2hb_s3only_main"
  image_id               = var.al2_hardened_ami_id
  instance_type          = var.k2hb_s3only_ec2_size[local.environment]
  ...
  user_data = base64encode(templatefile("k2hb_userdata.tpl", {
    ...
    k2hb_application_name     = local.k2hb_s3only_consumer_name
    ...
    cwa_log_group_name                               = aws_cloudwatch_log_group.k2hb_ec2_s3only_logs.name
    ...
    k2hb_kafka_topic_regex                           = local.kafka_consumer_s3only_topics_regex[local.environment]
  ...
  block_device_mappings {
    ...
    ebs {
      ...
      volume_type           = local.k2hb_s3only_dedicated_ebs_type[local.environment]
      delete_on_termination = true
      encrypted             = true
    }
  }

  tags = local.k2hb_s3only_london_tags_asg
...
resource "aws_autoscaling_group" "k2hb_s3only_london" {
  name_prefix               = "${aws_launch_template.k2hb_s3only_london.name}-lt_ver${aws_launch_template.k2hb_s3only_london.latest_version}_"
  min_size                  = local.k2hb_asg_min[local.environment]
  desired_capacity          = var.k2hb_s3only_london_asg_desired[local.environment]
  ...
  launch_template {
    id      = aws_launch_template.k2hb_s3only_london.id
    version = "$Latest"
  }

  tags = [
    for key, value in local.k2hb_s3only_london_tags_asg :
    {
      key                 = key
      value               = value
