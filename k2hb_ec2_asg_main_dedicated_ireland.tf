...
locals {
  k2hb_main_dedicated_ireland_tags_asg = merge(
    local.k2hb_main_tags_asg,
    {
      Name     = "k2hb-main-dedicated-ireland",
      Location = "Ireland",
    }
  )
}

resource "aws_launch_template" "k2hb_main_dedicated_ireland" {
  name                   = "k2hb_main_dedicated_ireland"
  ...
  user_data = base64encode(templatefile("k2hb_userdata.tpl", {
    environment_name          = "${local.environment}-dedicated-ireland"
  ...
  tags = local.k2hb_main_dedicated_ireland_tags_asg
...
resource "aws_autoscaling_group" "k2hb_main_dedicated_ireland" {
  name_prefix               = "${aws_launch_template.k2hb_main_dedicated_ireland.name}-lt_ver${aws_launch_template.k2hb_main_dedicated_ireland.latest_version}_"
  min_size                  = local.k2hb_asg_min[local.environment]
  desired_capacity          = var.k2hb_main_dedicated_ireland_asg_desired[local.environment]
  max_size                  = var.k2hb_main_dedicated_ireland_asg_max[local.environment]
  ...
  launch_template {
    id      = aws_launch_template.k2hb_main_dedicated_ireland.id
    version = "$Latest"
  }

  tags = [
    for key, value in local.k2hb_main_dedicated_ireland_tags_asg :
    {
      key                 = key
      value               = value
