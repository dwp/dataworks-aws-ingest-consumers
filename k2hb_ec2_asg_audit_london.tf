...
resource "aws_launch_template" "k2hb_audit_london" {
  ...
  user_data = base64encode(templatefile("k2hb_userdata.tpl", {
    ...
    k2hb_kafka_fetch_min_bytes                       = local.k2hb_audit_kafka_fetch_min_bytes[local.environment]
    k2hb_kafka_fetch_max_wait_ms                     = local.k2hb_audit_kafka_fetch_max_wait_ms[local.environment]
    k2hb_kafka_consumer_request_timeout_ms           = local.k2hb_audit_kafka_consumer_request_timeout_ms[local.environment]
  }))
