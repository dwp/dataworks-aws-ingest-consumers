...
resource "aws_launch_template" "k2hb_audit_london" {
  ...
  user_data = base64encode(templatefile("k2hb_userdata.tpl", {
    ...
    k2hb_write_to_metadata_store                     = local.k2hb_audit_london_write_to_metadata_store[local.environment]
    ...
    k2hb_write_manifests                             = local.k2hb_audit_london_write_manifests[local.environment]
    k2hb_auto_commit_metadata_store_inserts          = local.k2hb_audit_london_auto_commit_metadata_store_inserts[local.environment]
  }))
